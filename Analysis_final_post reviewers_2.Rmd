...
sepsis_samples$patient_type <- "Suspected sepsis"
...
```{r}
testing_datframe <- subset(overall, overall$patient_type=="Suspected sepsis")
...
png(
  "~/Documents/OneDrive - University of Cambridge/Documents/Sepsis_analysis/Sepsis_analysis/Output/Biomarker_correlations.png",
  width = 17,
  height = 17,
  units = "cm",
  res = 300
...
overall$patient_type[overall$patient_type == "Sepsis"] <- "Suspected sepsis"
...
GFAP_plot <- bxp + stat_compare_means(
  method = "wilcox.test", 
  label = "p.label", 
  comparisons = list(c("Suspected sepsis", "Non-Infected Control")), # Adjust according to your group names
 label.y = 7, 
  size = 10 
)
...
UCH_plot <- bxp + stat_compare_means(
  method = "wilcox.test", 
  label = "p.format", 
  comparisons = list(c("Suspected sepsis", "Non-Infected Control")), 
  label.y = 7,
  size = 10
)
  UCH_plot
...
```{r}
mean_demo <- subset(overall, overall$patient_type=="Suspected sepsis")
mean_demo<- merge(mean_demo, basic_demographics, by.x="Patient_ID", by.y="study_id")

mean_demo_lon <- subset(overall_longitudinal, overall_longitudinal$patient_type=="Suspected sepsis")
...
NFL_lactate_scatter <- ggscatter(mean_demo, x = "NFL", y = "pres_lactate",
  ...
  theme(strip.text = element_text(size = 18),
        ...
        legend.text = element_text(size = 18)) +
  ylim(0, 10) + xlim(0,90)
NFL_lactate_scatter <- NFL_lactate_scatter + annotate("text", x=30, y=9.5, label= "ρ = -0.16, p=0.46", size=6)
...
TAU_lactate_scatter<- ggscatter(mean_demo, x = "TAU", y = "pres_lactate",
  ...
  theme(strip.text = element_text(size = 18),
        ...
        legend.text = element_text(size = 18)) +
  ylim(0, 10) + xlim(0,20)
TAU_lactate_scatter <- TAU_lactate_scatter + annotate("text", x=7, y=9.5, label= "ρ = 0.15, p=0.48", size=6)
...
GFAP_news
GFAP_news <- GFAP_news + annotate("text", x=175, y=13, label= "ρ = -0.18, p=0.42", size=6)
...
NFL_news <- ggscatter(mean_demo, x = "NFL", y = "news_score",
  ...
  theme(strip.text = element_text(size = 18),
        ...
        legend.text = element_text(size = 18)) +
  ylim(4, 14) + xlim(0,90)
NFL_news <- NFL_news + annotate("text", x=30, y=13, label= "ρ = -0.22, p=0.32", size=6)
...
TAU_news<- ggscatter(mean_demo, x = "TAU", y = "news_score",
  ...
  theme(strip.text = element_text(size = 18),
        ...
        legend.text = element_text(size = 18)) +
  ylim(4, 14) + xlim(0,20)
TAU_news <- TAU_news + annotate("text", x=7, y=13, label= "ρ = 0.15, p=0.49", size=6)
...
bxp <- ggplot(mean_demo, aes(x = SOFA_Bin, y = logGFAP)) +
  geom_jitter(width = 0.2, size = 2, color = "black") +  # simple black dots
  labs(title = "GFAP", x = "qSOFA score", y = "log(GFAP) (pg/ml)") +
...
bxp <- ggplot(mean_demo, aes(x = SOFA_Bin, y = logNFL)) +
  geom_jitter(width = 0.2, size = 2, color = "black") +  # simple black dots
  labs(title = "NfL", x = "qSOFA score", y = "log(NfL) (pg/ml)") +
  scale_x_discrete(labels = c("0-1", "2-3")) +
  theme_classic() +
  theme(
    axis.text = element_text(size = 10),
...
bxp <- ggplot(mean_demo, aes(x = SOFA_Bin, y = logTau)) +
  geom_jitter(width = 0.2, size = 2, color = "black") +  # simple black dots
  labs(title = "total tau", x = "qSOFA score", y = "log(total tau) (pg/ml)") +
  scale_x_discrete(labels = c("0-1", "2-3")) +
  theme_classic() +
  theme(
    axis.text = element_text(size = 10),
...
bxp <- ggplot(mean_demo, aes(x = SOFA_Bin, y = logUCHL1)) +
  geom_jitter(width = 0.2, size = 2, color = "black") +  # simple black dots
  labs(title = "UCH-L1", x = "qSOFA score", y = "log(UCH-L1) (pg/ml)") +
  scale_x_discrete(labels = c("0-1", "2-3")) +
  theme_classic() +
  theme(
    axis.text = element_text(size = 10),
...
```{r}
pca_wide <- sourcedata_cytokines_wide[, -c(1:6)]
pca_wide <- pca_wide[, -c(19:23)]
...
```{r}
library(factoextra)
pca_df <- subset(pca_wide, pca_wide$timepoint=="T1")
eid_column <- pca_df$study_ID

pca_data <- pca_df %>% select("TNF-alpha":"CCL4")
...
GFAP <- ggplot(GFAP_T1_correlation, aes(x = GFAP, y = log_p)) +
  geom_point(aes(color = p_adj < 0.01), size = 1.5) +  
  ...
  theme(axis.text = element_text(size = 10),
        ...
        legend.position = "none") + xlim(-0.5,1) + ylim(0,3)
    GFAP    
```
...
UCH
All_plot <- cowplot::plot_grid(GFAP, NFL,Tau,UCH, ncol = 2,  labels = c("A", "B", "C", "D"),
  label_size = 14)
...
pTau$patient_type[pTau$patient_type == "Sepsis"] <- "Suspected sepsis"

pTau_patients <- subset(pTau, pTau$patient_type=="Suspected sepsis")
...
test_Tau <- subset(test_Tau, test_Tau$patient_type.x=="Suspected sepsis" | test_Tau$patient_type.x == "BioResource Control")
...
ptautauratio <-  bxp+ stat_compare_means(
  method = "wilcox.test", 
  label = "p.format", 
  comparisons = list(c("Suspected sepsis", "BioResource Control")), 
  label.y = 0.4,
  size = 10
)
 ptautauratio 
...
```{r}
test_Tau_sepsis <- subset(test_Tau, test_Tau$patient_type.x=="Suspected sepsis")
...
test_df <- subset(pTau, pTau$patient_type=="Suspected sepsis" | pTau$patient_type == "Non-Infected control" | pTau$patient_type == "Alzhiemers Control")
...
dunn.test(test_df$pTau217, test_df$patient_type, method = "BH")
...
test_df$patient_type <- factor(test_df$patient_type, 
                               levels = c("Non-Infected control", "Suspected sepsis", "Alzheimer's Control"))
...
custom_colors <- c(
  "Low" = "#0CB702",         
  "Intermediate" = "darkgoldenrod", 
  "High" = "#F8766D"         
)

summary_table$patient_type_label <- factor(summary_table$patient_type_label, levels = c("Non-Infected control (n=20)", "Suspected sepsis (n=24)", "Alzheimer's Control (n=9)"))
...
```{r}
sepsis_check <- subset(test_df, test_df$patient_type=="Suspected sepsis")
